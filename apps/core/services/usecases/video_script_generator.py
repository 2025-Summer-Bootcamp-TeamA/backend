...
5. **적절한 길이**: 5-10초 정도의 짧은 영상에 맞는 분량
6. **한국어**: 자연스러운 한국어로 작성
...
- 감정적이고 몰입감 있는 톤으로 작성
- **중요**: 5-10초 분량으로 간결하고 핵심적인 내용만 포함

길이 가이드:
- 한국어 나레이션 기준 약 200-250자 정도 (5-10초 분량)
...
응답은 스크립트 내용만 작성하세요. 다른 설명이나 주석은 포함하지 마세요."""
    ...
    def _calculate_script_length(self, script_content: str) -> int:
        ...
        # 5-10초 범위로 제한 (목표 길이)
        return max(5, min(10, estimated_seconds))
