...
class GCSStorageService:
    ...
    @staticmethod
    def _process_file(source_url: str, source_folder: str, target_folder: str, delete_source: bool = False) -> Optional[str]:
        """
        GCS 파일 처리 공통 로직
        ...
        Args:
            ...
            delete_source (bool): 원본 파일 삭제 여부
            
        Returns:
            Optional[str]: 처리된 파일의 새 URL 또는 None (실패 시)
        """
        try:
            # URL에서 파일 경로 추출
            ...
            if default_storage.exists(file_path):
                ...
                if delete_source:
                    try:
                        ...
                        logger.error(f"원본 파일 삭제 실패: {e}")
                        # 롤백 시도
                        try:
                            default_storage.delete(target_gcs_path)
                        except Exception:
                            pass
                        raise
                else:
            ...
            operation = "이동" if delete_source else "복사"
            logger.error(f"GCS 파일 {operation} 실패: {e}")
            return None
    ...
    def move_file(source_url: str, source_folder: str, target_folder: str) -> Optional[str]:
        ...
        """
        return GCSStorageService._process_file(source_url, source_folder, target_folder, delete_source=True)
    ...
    def copy_file(source_url: str, source_folder: str, target_folder: str) -> Optional[str]:
        ...
        """
        return GCSStorageService._process_file(source_url, source_folder, target_folder, delete_source=False)
